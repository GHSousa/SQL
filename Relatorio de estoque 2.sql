...
FROM
			...
			AND (CASE WHEN TMOV.DATACRIACAO IS NULL THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) >= '2023-01-01' 
			AND (CASE WHEN TMOV.DATACRIACAO IS NULL THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) <= '2023-05-01'
			AND (TITMMOV.QUANTIDADE<>0 OR TITMMOV.QUANTIDADETOTAL<>0)
			...
			AND TMOV.CODLOC >='3.0055' 
			AND TMOV.CODLOC <='3.0055' 
			AND TPRD.CODIGOPRD >='a' 
			AND TPRD.CODIGOPRD <='z' 
			AND (CASE WHEN TMOV.DATACRIACAO IS NULL THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) >= '2023-01-01' 
			AND (CASE WHEN CONVERT(DATE,TMOV.DATACRIACAO) IS NULL THEN CONVERT(DATE,TMOV.DATACRIACAO) ELSE  CONVERT(DATE,TMOV.DATACRIACAO) END) <= '2023-05-01'
			...
			AND TMOV.CODTMV IN (
				select TTMV.CODTMV 
				from TTMV (NOLOCK) 
				inner join TITMTMV (NOLOCK) on TTMV.CODTMV = TITMTMV.CODTMV AND TTMV.CODCOLIGADA = TITMTMV.CODCOLIGADA and TITMTMV.codtmv ='4.1.03')  
			AND TMOV.CODLOC >='3.0055' 
			AND TMOV.CODLOC <='3.0055' 
			AND TPRD.CODIGOPRD >='a' 
			AND TPRD.CODIGOPRD <='z' 
