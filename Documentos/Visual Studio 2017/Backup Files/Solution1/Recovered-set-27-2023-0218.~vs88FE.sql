	...
	DECLARE @TAMANHO_EMP INT = (SELECT COUNT (*) FROM XEMPREENDIMENTO (NOLOCK) WHERE XEMPREENDIMENTO.codcoligada = 22)
	DECLARE @RESULTADO_EMP VARCHAR(MAX) =''
	DECLARE @CONT_PARC INT = 0
	DECLARE @TAMANHO_PARC INT = (SELECT COUNT (*) FROM XTIPOPARCELA (NOLOCK) WHERE XTIPOPARCELA.COD_TIPO_PARC <> 0 )
	DECLARE @RESULTADO_PARC VARCHAR(MAX) =''
	DECLARE @EMPREENDIMENTO INT = (SELECT COUNT (*) FROM XEMPREENDIMENTO (NOLOCK) WHERE XEMPREENDIMENTO.codcoligada = 22)
	DECLARE @TIPO_PARC INT = (SELECT COUNT (*) FROM XTIPOPARCELA (NOLOCK) WHERE XTIPOPARCELA.COD_TIPO_PARC <> 0 )
	DECLARE @NOME_CLIENTE NVARCHAR(255) 
	SET @NOME_CLIENTE = (SELECT TOP 1 FCFO.NOMEFANTASIA FROM FCFO WHERE FCFO.NOMEFANTASIA LIKE '%RONALD%')
	DECLARE @DATA_PAGAMENTO_INI DATE = ('20200101')
	DECLARE @DATA_PAGAMENTO_FIN DATE = ('20230926')
	DECLARE @DATA_VENCIMENTO_INI DATE = ('20200101')
	DECLARE @DATA_VENCIMENTO_FIN DATE =('20230926')
