...
SET @DATA_BASE_D = @DATA_BASE_JUROS_MULTA
SET @MULTA='0.02'
...
WHERE				XEMPREENDIMENTO.COD_PESS_EMPR IN (@EMPREENDIMENTO)
					AND FLAN.STATUSLAN IN (0,4)
					AND FLAN.DATAVENCIMENTO BETWEEN @VENCIMENTO_INI AND @VENCIMENTO_FIN
					AND FCFO.NOME LIKE '%' + @NOME_CLIENTE + '%' 
					AND XTIPOPARCELA.COD_TIPO_PARC IN (@TIPO_PARC)
