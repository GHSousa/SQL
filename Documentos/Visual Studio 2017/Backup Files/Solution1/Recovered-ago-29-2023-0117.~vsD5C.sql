...
WHERE   
   ...
   ELSE TMOV.CODFILIAL END) ='1'
                ...
                AND (CASE WHEN TMOV.DATACRIACAO IS NULL 
                  THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) >= :ESPELHO#9
        AND (CASE WHEN TMOV.DATACRIACAO IS NULL 
                  THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) <= :ESPELHO#10
...
AND TPRD.CODIGOPRD >=:PRODUTOINICIAL AND TPRD.CODIGOPRD <=:PRODUTOFINAL AND 
(CASE WHEN TMOV.DATACRIACAO IS NULL THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) >= :DATAINICIAL_D AND
(CASE WHEN TMOV.DATACRIACAO IS NULL THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) <= :DATAFINAL_D
AND (TITMMOV.QUANTIDADE<>0
			OR TITMMOV.QUANTIDADETOTAL<>0)
...
TPRD.CODIGOPRD >=:PRODUTOINICIAL AND TPRD.CODIGOPRD <=:PRODUTOFINAL AND 
(CASE WHEN TMOV.DATACRIACAO IS NULL THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) >= :DATAINICIAL_D AND
(CASE WHEN CONVERT(DATE,TMOV.DATACRIACAO) IS NULL THEN CONVERT(DATE,TMOV.DATACRIACAO) ELSE  CONVERT(DATE,TMOV.DATACRIACAO) END) <= :DATAFINAL_D
