...
ISNULL((SELECT CAST(SUM(T.VAL)/CASE WHEN SUM(T.QUANTIDADE) = '0' THEN '0.01' ELSE SUM(T.QUANTIDADE)END AS DECIMAL (10,2)) AS CUSTOMEDIODEENTRADA
       FROM (
...
WHERE   
   ...
   ELSE TMOV.CODLOC END) BETWEEN  'a' AND 'z'
   ...
   ELSE TMOV.CODFILIAL END) ='1'
                ...
                AND (CASE WHEN TMOV.DATACRIACAO IS NULL 
                  THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) >= '2023-01-01'
        AND (CASE WHEN TMOV.DATACRIACAO IS NULL 
                  THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) <= '2023-08-28'
...
AND TPRD.CODIGOPRD >='a' AND TPRD.CODIGOPRD <='g' AND 
(CASE WHEN TMOV.DATACRIACAO IS NULL THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) >= '2023-01-01' AND
(CASE WHEN TMOV.DATACRIACAO IS NULL THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) <= '2023-08-28'
AND (TITMMOV.QUANTIDADE<>0
			OR TITMMOV.QUANTIDADETOTAL<>0)
...
INNER JOIN TPRD (NOLOCK) ON TPRD.CODCOLIGADA = TMOV.CODCOLIGADA AND TPRD.IDPRD = TITMMOV.IDPRD
 ...
 AND TMOV.CODTMV IN  ('1.1.03','1.1.27','1.1.33','3.1.28','3.1.36','3.1.39','2.1.01','4.1.06','4.1.05') AND TMOV.CODLOC >='a' AND TMOV.CODLOC <='z' AND
TPRD.CODIGOPRD >='a' AND TPRD.CODIGOPRD <='z' AND 
(CASE WHEN TMOV.DATACRIACAO IS NULL THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) >= '2023-01-01' AND
(CASE WHEN CONVERT(DATE,TMOV.DATACRIACAO) IS NULL THEN CONVERT(DATE,TMOV.DATACRIACAO) ELSE  CONVERT(DATE,TMOV.DATACRIACAO) END) <= '2023-08-28'
...
inner join TITMTMV (NOLOCK) on TTMV.CODTMV = TITMTMV.CODTMV AND TTMV.CODCOLIGADA = TITMTMV.CODCOLIGADA
 and TITMTMV.codtmv ='4.1.03')  AND TMOV.CODLOC >='a' AND TMOV.CODLOC <='z' AND 
TPRD.CODIGOPRD >='a' AND TPRD.CODIGOPRD <='z' AND 
(CASE WHEN TMOV.DATACRIACAO IS NULL THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) >= '2023-01-01' AND
(CASE WHEN TMOV.DATACRIACAO IS NULL THEN TMOV.DATACRIACAO ELSE TMOV.DATACRIACAO END) <= '2023-08-28'
) 
