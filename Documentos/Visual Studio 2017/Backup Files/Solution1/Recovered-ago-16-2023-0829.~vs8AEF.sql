...
(CASE WHEN TMOV.CODTMV = '3.1.28' THEN TMOV.CODLOCDESTINO  
   ELSE TMOV.CODLOC END) BETWEEN  '3.0055' AND '3.0055' AND 

TPRD.CODIGOPRD >='A' AND TPRD.CODIGOPRD <='Z' AND 
(CASE WHEN CAST(TMOV.DATACRIACAO AS DATE) IS NULL THEN CAST(TMOV.DATACRIACAO AS DATE) ELSE CAST(TMOV.DATACRIACAO AS DATE) END) >= :DATAINICIAL_D AND
(CASE WHEN CAST(TMOV.DATACRIACAO AS DATE) IS NULL THEN CAST(TMOV.DATACRIACAO AS DATE) ELSE CAST(TMOV.DATACRIACAO AS DATE) END) <= :DATAFINAL_D
...
INNER JOIN TPRD (NOLOCK) ON TPRD.CODCOLIGADA = TMOV.CODCOLIGADA AND TPRD.IDPRD = TITMMOV.IDPRD
 ...
 TMOV.CODCOLIGADA= '1'
 		AND (TITMMOV.QUANTIDADETOTAL<>0 
			OR TITMMOV.QUANTIDADE<>0) AND
 TMOV.STATUS <> 'C' AND TMOV.CODTMV IN  ('1.1.27','1.1.33','3.1.28','3.1.36','3.1.39','2.1.01','4.1.06','4.1.05') AND TMOV.CODLOC >='3.0055' AND TMOV.CODLOC <='3.0055' AND
TPRD.CODIGOPRD >='A' AND TPRD.CODIGOPRD <='Z' AND 
(CASE WHEN CAST(TMOV.DATACRIACAO AS DATE) IS NULL THEN CAST(TMOV.DATACRIACAO AS DATE) ELSE CAST(TMOV.DATACRIACAO AS DATE) END) >= :DATAINICIAL_D AND
(CASE WHEN CAST(TMOV.DATACRIACAO AS DATE) IS NULL THEN CAST(TMOV.DATACRIACAO AS DATE) ELSE CAST(TMOV.DATACRIACAO AS DATE) END) <= :DATAFINAL_D
...
LEFT JOIN (SELECT * FROM DBO.TABELA_PRECO_MEDIO('1', :DATAINICIAL_D, :DATAFINAL_D, '3.0055', '3.0055')) AS TAB_PRECO_MEDIO ON (TAB_PRECO_MEDIO.IDPRD = T.IDPRD AND TAB_PRECO_MEDIO.CODCOLIGADA = '1')
