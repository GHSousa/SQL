	...
	DECLARE @TAMANHO_EMP INT = (SELECT COUNT (*) FROM XEMPREENDIMENTO (NOLOCK) WHERE COD_PESS_EMPR IN (17586))
	DECLARE @RESULTADO_EMP VARCHAR(MAX) =''
	DECLARE @CONT_PARC INT = 0
	DECLARE @TAMANHO_PARC INT = (SELECT COUNT (*) FROM XTIPOPARCELA (NOLOCK) WHERE XTIPOPARCELA.COD_TIPO_PARC IS NULL)
			...
			SET @RESULTADO_PARC = @RESULTADO_PARC + (
											SELECT CONVERT (VARCHAR(10),COD_TIPO_PARC) FROM 
												(
													...
													WHERE XTIPOPARCELA.COD_TIPO_PARC IS NULL
