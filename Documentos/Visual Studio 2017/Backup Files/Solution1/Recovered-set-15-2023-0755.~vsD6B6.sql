...
TAB_PRECO_MEDIO.PRECO_MEDIO
 ...
 from 
(select TPRD.IDPRD, TPRD.CODIGOPRD, TPRD.NOMEFANTASIA, TMOV.CODTMV, 
...
TPRD.CODUNDCONTROLE,(CASE WHEN TMOV.CODTMV = '3.1.28' THEN TMOV.CODLOCDESTINO  ELSE TMOV.CODLOC END) AS CODLOC,0 AS ENTRADA2,TMOV.CODFILIAL
from TITMMOV (NOLOCK)
...
INNER JOIN TPRD (NOLOCK) ON TPRD.CODCOLIGADA = TMOV.CODCOLIGADA AND TPRD.IDPRD = TITMMOV.IDPRD
 ...
 TMOV.CODCOLIGADA= '29'  
 		AND (TITMMOV.QUANTIDADETOTAL<>0 
			OR TITMMOV.QUANTIDADE<>0) AND 
 TMOV.STATUS <> 'C' AND TMOV.CODTMV IN ('2.2.05','1.1.40','1.1.41','4.1.01','4.1.04' ,'3.1.28')AND

(CASE WHEN TMOV.CODTMV = '3.1.28' THEN TMOV.CODLOCDESTINO  
   ELSE TMOV.CODLOC END) BETWEEN  '0.0001' AND '9.9999' AND 

TPRD.CODIGOPRD >='a' AND TPRD.CODIGOPRD <='z' AND 
(CASE WHEN CAST(TMOV.DATACRIACAO AS DATE) IS NULL THEN CAST(TMOV.DATACRIACAO AS DATE) ELSE CAST(TMOV.DATACRIACAO AS DATE) END) >= '20200101' AND
(CASE WHEN CAST(TMOV.DATACRIACAO AS DATE) IS NULL THEN CAST(TMOV.DATACRIACAO AS DATE) ELSE CAST(TMOV.DATACRIACAO AS DATE) END) <= '20230901'
...
INNER JOIN TPRD (NOLOCK) ON TPRD.CODCOLIGADA = TMOV.CODCOLIGADA AND TPRD.IDPRD = TITMMOV.IDPRD
 ...
 TMOV.CODCOLIGADA= '29'
 		AND (TITMMOV.QUANTIDADETOTAL<>0 
			OR TITMMOV.QUANTIDADE<>0) AND
 TMOV.STATUS <> 'C' AND TMOV.CODTMV IN  ('1.1.27','1.1.33','3.1.28','3.1.36','3.1.39','2.1.01','4.1.06','4.1.05') AND TMOV.CODLOC >='0.0001' AND TMOV.CODLOC <='9.9999' AND
TPRD.CODIGOPRD >='a' AND TPRD.CODIGOPRD <='z' AND 
(CASE WHEN CAST(TMOV.DATACRIACAO AS DATE) IS NULL THEN CAST(TMOV.DATACRIACAO AS DATE) ELSE CAST(TMOV.DATACRIACAO AS DATE) END) >= '20200101' AND
(CASE WHEN CAST(TMOV.DATACRIACAO AS DATE) IS NULL THEN CAST(TMOV.DATACRIACAO AS DATE) ELSE CAST(TMOV.DATACRIACAO AS DATE) END) <= '20230901'
...
(CASE WHEN TITMMOV.CODUND = TPRD.CODUNDCONTROLE 
     ...
     *
     (SELECT FATORCONVERSAO FROM TUND (NOLOCK) WHERE TUND.CODUND = TITMMOV.CODUND))
     / (SELECT FATORCONVERSAO FROM TUND (NOLOCK) WHERE TUND.CODUND = TPRD.CODUNDCONTROLE)
     END) AS ENTRADA2,TMOV.CODFILIAL
from TITMMOV (NOLOCK)
INNER JOIN TMOV (NOLOCK) ON TMOV.CODCOLIGADA = TITMMOV.CODCOLIGADA AND TMOV.IDMOV = TITMMOV.IDMOV
INNER JOIN TPRD (NOLOCK) ON TPRD.CODCOLIGADA = TMOV.CODCOLIGADA AND TPRD.IDPRD = TITMMOV.IDPRD
 where 
 TMOV.CODCOLIGADA= '29'
 		AND (TITMMOV.QUANTIDADETOTAL<>0 
			OR TITMMOV.QUANTIDADE<>0) AND 
 TMOV.STATUS <> 'C' AND TMOV.CODTMV IN (select TTMV.CODTMV from TTMV (NOLOCK)
inner join TITMTMV (NOLOCK) on TTMV.CODTMV = TITMTMV.CODTMV AND TTMV.CODCOLIGADA = TITMTMV.CODCOLIGADA
 and TITMTMV.codtmv ='4.1.03')  AND TMOV.CODLOC >='0.0001' AND TMOV.CODLOC <='9.9999' AND 
TPRD.CODIGOPRD >='a' AND TPRD.CODIGOPRD <='z' AND 
(CASE WHEN CAST(TMOV.DATACRIACAO AS DATE) IS NULL THEN CAST(TMOV.DATACRIACAO AS DATE) ELSE CAST(TMOV.DATACRIACAO AS DATE) END) >= '20200101' AND
(CASE WHEN CAST(TMOV.DATACRIACAO AS DATE) IS NULL THEN CAST(TMOV.DATACRIACAO AS DATE) ELSE CAST(TMOV.DATACRIACAO AS DATE) END) <= '20230901'
) 
...
LEFT JOIN (SELECT * FROM DBO.TABELA_PRECO_MEDIO('29', '20200101', '20230901', '0.0001', '9.9999')) AS TAB_PRECO_MEDIO ON (TAB_PRECO_MEDIO.IDPRD = T.IDPRD AND TAB_PRECO_MEDIO.CODCOLIGADA = '29')
